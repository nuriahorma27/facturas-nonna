...
function ViewConciliacion({ facturas, toast }) {
  ...
  const sincronizarWoo = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        let detail = "";
        try { detail = JSON.parse(text)?.message || ""; } catch(_) {}
        throw new Error(detail || `Error ${res.status} — revisa Ejecuciones en n8n`);
      }
      if (!text || !text.trim()) {
        throw new Error("Sin respuesta del workflow — revisa Ejecuciones en n8n");
      }
      ...
      if (d.success) {
        ...
        throw new Error(d.error || "Error en el workflow — revisa Ejecuciones en n8n");
      }
    } catch(e) {
...
// ═══════════════════════════════════════════════════════════
const LOGIN_CSS = `
.login-wrap{min-height:100vh;background:#EDE5D0;display:flex;align-items:center;justify-content:center}
.login-box{background:#F5F0E8;border:.5px solid #D4C5A9;padding:56px 48px;width:100%;max-width:400px;animation:fadeUp .5s ease both}
.login-mono{width:48px;height:48px;border:1.5px solid #B8962E;display:flex;align-items:center;justify-content:center;color:#B8962E;font-size:18px;margin:0 auto 20px}
.login-title{font-size:28px;font-weight:300;text-align:center;margin-bottom:4px;color:#2C2417}
.login-sub{font-size:13px;letter-spacing:.25em;text-transform:uppercase;color:#9C8E7A;text-align:center;margin-bottom:36px}
.login-field{display:flex;flex-direction:column;gap:6px;margin-bottom:22px}
.login-lbl{font-size:13px;letter-spacing:.2em;text-transform:uppercase;color:#5C4A2A}
.login-inp{background:transparent;border:none;border-bottom:1.5px solid #D4C5A9;font-family:'Cormorant Garamond',serif;font-size:18px;color:#2C2417;outline:none;padding:8px 0;width:100%;transition:border-color .2s}
.login-inp:focus{border-bottom-color:#B8962E}
...
function LoginScreen({ onLogin }) {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      if (pwd === PASS) {
        sessionStorage.setItem("atelier_auth", "1");
        onLogin();
      } else {
  ...
  return (
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,500;0,600;1,300;1,400&display=swap');*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Cormorant Garamond',Georgia,serif}@keyframes fadeUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <style>{LOGIN_CSS}</style>
      <div className="login-wrap">
        <div className="login-box">
          <div className="login-mono">N</div>
          <div className="login-title">Atelier La Nonna</div>
          <div className="login-sub">Gestor de facturas</div>
          ...
          >
            {loading ? <><span className="spin" style={{width:16,height:16}}/><span>Verificando</span></> : <span>Entrar</span>}
          </button>
        </div>
      </div>
    </>
...
export default function AtelierApp() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("atelier_auth") === "1");
